...
func _physics_process( delta : float ) -> void:
	...
	var rotation : float = Input.get_action_strength("TURN_RIGHT") - Input.get_action_strength("TURN_LEFT")
	if( rotation != 0 ):
		goRotation( rotation , delta )

	#var acceleration : float = Input.get_action_strength("")
